...
- **Parenthetical** objects are extracted from the opinion text when a decision explains a citation it relies on as authority. These are not [yet available in an API][parentheticals-issue], but are available as bulk data.

[case-law-coverage]: https://www.courtlistener.com/help/coverage/opinions/
[citation-api]: #courtlistener/help/api/rest/v4/citations
[parentheticals-issue]: https://github.com/freelawproject/courtlistener/issues/4082

## The APIs

### Dockets
[/api/rest/v4/dockets/](https://www.courtlistener.com/api/rest/v4/dockets/){button}
...
Docket objects sit at the top of the object hierarchy:
 ...
 - In our oral argument database, they sit above audio objects.

To look up field descriptions or options for [filtering, ordering, or serialization](#courtlistener/help/api/rest/v4/query-refinement), do an HTTP `OPTIONS` request:
...
The name of a case can differ from the name of a decision within it. Therefore, we have `case_name` fields on both the docket and the cluster. When using the case law API, use the case name fields defined on the cluster object.

For example, a suit filed against the EPA administrator might be captioned *Petroleum Co. v. Regan*. That would go into the case name fields of the docket and any decisions that were issued. But if the administrator resigns before the case is resolved, the docket could get a new case name, *Petroleum Co. v. New Administrator*, while the case name fields on the clusters would not change.

For more information, see the [advanced help on case names][case-names-help].

[case-names-help]: #courtlistener/help/api/rest/v4/field-help#case-names

### Clusters
[/api/rest/v4/clusters/](https://www.courtlistener.com/api/rest/v4/clusters/){button}
...
- The `id` field of the cluster is used in opinion URLs on CourtListener.
...
- There are several fields with judge information, such as `judges`, `panel`, `non_participating_judges`, etc. Some of these fields contain strings and others are linked to records in our [judge API][judge-api]. 

  When we are able to normalize a judge's name into a record in the judge database, we do so. If not, we store their name in a string field for later normalization.

[judge-api]: #courtlistener/help/api/rest/v4/judges

### Opinions
[/api/rest/v4/opinions/](https://www.courtlistener.com/api/rest/v4/opinions/){button}
...
> [!WARNING]
> When retrieving opinion text, prefer the `html_with_citations` field instead of `plain_text`. The `html_with_citations` field contains the raw text of the decision, and is the most reliable field for most purposes.
>
...
- The `opinions_cited` field has a list of other opinions cited by the one you are reviewing (like an authorities table).
...
[file-downloads-help]: #courtlistener/help/api/rest/v4/field-help#file-download-fields
[field-selection]: #courtlistener/help/api/rest/v4/query-refinement#field-selection

### Courts
[/api/rest/v4/courts/](https://www.courtlistener.com/api/rest/v4/courts/){button}
...
[search-api]: #courtlistener/help/api/rest/v4/search
