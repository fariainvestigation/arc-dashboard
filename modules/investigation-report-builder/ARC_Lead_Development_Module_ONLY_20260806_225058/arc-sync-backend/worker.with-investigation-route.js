...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      const investigationMatch = path.match(/^\/sync\/case\/([A-Za-z0-9_.:-]+)\/investigation$/);
      if (investigationMatch) {
        const id = investigationMatch[1];
        const caseRow = await env.ARC_DB.prepare(
          "SELECT id FROM cases WHERE id = ? AND deleted = 0"
        )
          .bind(id)
          .first();
        if (!caseRow) return json({ error: "Case not found" }, 404, cors);

        if (request.method === "GET") {
          const row = await env.ARC_DB.prepare(
            "SELECT data, updated_at, updated_by FROM investigation_workspaces WHERE case_id = ?"
          )
            .bind(id)
            .first();
          if (!row) {
          ...
          workspace.savedAt = row.updated_at;
          workspace.modifiedBy = row.updated_by;
          return json(workspace, 200, cors);
        }

        if (request.method === "PUT") {
          const body = await request.json().catch(() => null);
          if (!body || typeof body !== "object" || Array.isArray(body)) {
            return json({ error: "Invalid investigation workspace body" }, 400, cors);
          }
          ...
          await env.ARC_DB.prepare(
            `INSERT INTO investigation_workspaces (case_id, data, updated_at, updated_by)
             VALUES (?, ?, ?, ?)
            ...
            .bind(id, JSON.stringify(clean), now, user)
            .run();
