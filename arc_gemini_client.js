...
(function (global) {
  ...
  function activeCaseId() {
    ...
    try { if (global.ARCUnified && global.ARCUnified.getCase) { var c=global.ARCUnified.getCase()||{}; return String(c.caseId||c.id||''); } } catch (e) {}
    return '';
  }
  function gatewayHeaders(extra) {
    var h = Object.assign({}, extra || {});
    var cid = activeCaseId();
    if (cid) h['X-ARC-Case-ID'] = cid;
    return h;
  }
  ...
  function waitForActive(fileRef, report) {
    ...
    function poll() {
      ...
      return new Promise(function (r) { setTimeout(r, 2000); }).then(function () {
        return fetch(BASE + "/v1beta/" + name, { credentials: "include", headers: gatewayHeaders() });
      }).then(function (response) {
  ...
  function deleteFile(name) {
    ...
    return fetch(BASE + "/v1beta/" + name, { method: "DELETE", credentials: "include", headers: gatewayHeaders() })
