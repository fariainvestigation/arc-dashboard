(function () {
  ...
  async function renderDashboard() {
    setCaseHeader();
    let all = [];
    try { all = await window.ARCReportAssets.list(); } catch (error) {}
    const definitions = dashboard === "reports" ? [
      ["Pictures", "12_Report_Pictures.html", "picture", "Approved photographs, captions, and exhibit order."],
      ["Videos", "13_Report_Videos.html", "video", "Source video uploads, timestamps, and explanations."],
      ["Transcripts", "14_Report_Transcripts.html", "transcript", "Speakers, timestamps, corrections, and excerpts."],
      ["Translations", "15_Report_Translations.html", "translation", "Original and translated text with verification context."],
      ["Full Motions", "16_Report_Full_Motions.html", "motion", "Completed motion documents ready for review."],
      ["Attorney Manifest", "#", "manifest", "Approved selections prepared for the future report viewer JSON."]
    ] : dashboard === "evidence" ? [
      ["Evidence Notebook", "arc-notebook.html", "", "Import and analyze source records."],
      ["Discovery Review", "04_Discovery_Review.html", "", "Review production and identify missing material."],
      ["Chain of Custody", "05_Chain_of_Custody_Analyzer.html", "", "Verify transfers, hashes, and exhibit integrity."],
      ["Master Timeline", "06_Timeline_Discrepancy.html", "", "Build case-specific chronology and discrepancies."],
      ["Investigation Rebuilt", "07_Intelligence_Slides_Analysis_Investigator.html", "", "Manage parties, evidence, leads, contradictions, and reports."],
      ["Investigation Map", "map/index.html", "", "Plot evidence, routes, and location findings."]
    ] : [
      ["Forensic Video Discovery", "https://forensic-video-discovery-analyzer.ai.studio/", "", "Open the external forensic video workspace."],
      ["ARC OSINT", "https://arc-investigations-osint.ai.studio/", "", "Open the external public-source research workspace."],
      ["OUI / Drug Defense", "https://arc-oui-and-drug-defense-analyzer.ai.studio/", "", "Open the external OUI and drug-defense analyzer."]
    ];
    document.getElementById("rwDashboard").innerHTML = definitions.map(item => {
      const count = item[2] === "manifest" ? all.filter(record => record.selectedForFinal && ["approved", "included"].includes(record.status)).length : all.filter(record => record.type === item[2]).length;
      return "<a class='rw-dash-link' href='" + esc(item[1]) + "' " + (/^https?:/.test(item[1]) ? "target='_blank' rel='noopener'" : "") + " data-kind='" + esc(item[2]) + "'><h2>" + esc(item[0]) + "</h2><p>" + esc(item[3]) + "</p>" + (dashboard === "reports" ? "<strong>" + count + "</strong>" : "") + "</a>";
    }).join("");
    document.querySelector("[data-kind='manifest']")?.addEventListener("click", function (event) { event.preventDefault(); window.ARCReportAssets.downloadManifest(); });
  }
