(function () {
  ...
  // Canonical case id only. The docket and matter fallbacks were removed:
  // two matters sharing a docket would share assets, and any case that
  // acquired a canonical id later would orphan everything filed under its
  // docket. An asset with no canonical case id is not filed at all.
  const CANONICAL_CASE_ID = /^[A-Za-z0-9_-]{4,64}$/;

  function caseId() {
    const value = String((caseContext() || {}).caseId || "").trim();
  ...
  function cleanRecord(value) {
    ...
    return Object.assign({}, source, {
      ...
      status: status,
      // A source document can never be marked for the final report while it
      // is still a source document. Promote it to an exhibit type first.
      selectedForFinal: SOURCE_TYPES.includes(type) ? false : Boolean(source.selectedForFinal),
  ...
  window.ARCReportAssets = {
    EXHIBIT_TYPES: EXHIBIT_TYPES.slice(),
    SOURCE_TYPES: SOURCE_TYPES.slice(),
    isSourceType: function (type) { return SOURCE_TYPES.indexOf(String(type)) >= 0; },
