...
function makeEnv(overrides = {}) {
  ...
  db.prepare("INSERT INTO cases (id,data,rev,deleted,updated_at,updated_by,owner_user_id) VALUES (?,?,?,?,?,?,?)")
    .run(TEST_CASE, JSON.stringify({ caseId: TEST_CASE }), 1, 0, 'now', 'raf@arc.test', 'raf@arc.test');
  db.prepare("INSERT INTO case_members (case_id,user_id,role,granted_by,granted_at) VALUES (?,?,?,?,?)")
    .run(TEST_CASE, 'raf@arc.test', 'editor', 'raf@arc.test', 'now');

  return {
    db,
    GOOGLE_API_KEY: 'gemini-test-key-DO-NOT-LEAK',
    ANTHROPIC_API_KEY: 'TEST_ANTHROPIC-key-DO-NOT-LEAK',
...
test('no provider key appears in any gateway response', async () => {
  ...
  try {
    for (const task of ['report.final.draft', 'evidence.extract']) {
      ...
      assert.ok(!raw.includes('TEST_ANTHROPIC-key'), task + ' leaked the Claude key');
