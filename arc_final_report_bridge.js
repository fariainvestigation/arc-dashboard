(function () {
  ...
  function reportTargetOrigin() {
    try {
      var url = new URL(TEMPLATE_URL, location.href);
  ...
  async function openReport(caseItem, options) {
    ...
    try {
      ...
      const deliverPayload = function (event) {
        ...
        target.postMessage({ type: "ARC_FINAL_REPORT_PAYLOAD", payload: payload }, reportTargetOrigin());
